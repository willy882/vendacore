          ...
          <div className="grid grid-cols-2 gap-3 bg-slate-50 rounded-xl p-4">
            ...
            <div><p className="text-xs text-slate-500">Estado</p>
              <span className="inline-flex items-center gap-1">
                <Badge variant={STATUS_MAP[doc.estado]?.variant ?? 'default'}>{STATUS_MAP[doc.estado]?.label ?? doc.estado}</Badge>
              </span>
            </div>
              ...
              {isLoading ? (
                ...
                return (
                  <tr key={doc.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => setDetailId(doc.id)} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Ver detalle">
                        <Eye size={15} />
                      </button>
                    </td>
                  </tr>
